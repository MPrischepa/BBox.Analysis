...
namespace BBox.Analysis.Domain.RecordTemplates.FuelStationTemplates
{
    internal class BonusesChargeTemplate : RecordTemplate<FuelStation>
    {
        ...
        public override ProcessingResult Process(FuelStation entity, Record record)
        {
            ...
            if (!(__sale.Payment is BonusCardPayment __payment)) throw new Exception("Ожидается основание Бонусы");
            __pattern = "Начисление бонусов:\\s+\\d+,\\d{2}";
            ...
            var __ekstra = !String.IsNullOrWhiteSpace(__str)
                ? Decimal.Parse(new Regex("\\d+,\\d{2}").Match(__str).Value, __numberFormatInfo)
                : 0;
